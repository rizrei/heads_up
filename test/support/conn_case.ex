defmodule HeadsUpWeb.ConnCase do
  ...
  by setting `use HeadsUpWeb.ConnCase, async: true`, although
  ...
  def register_and_log_in_user(%{conn: conn} = context) do
    ...
    scope = HeadsUp.Accounts.Scope.for_user(user)
  ...
  def register_and_log_in_admin(%{conn: conn} = context) do
    user = HeadsUp.AccountsFixtures.admin_fixture()
  ...
  def log_in_user(conn, user, opts \\ []) do
    token = HeadsUp.Accounts.generate_user_session_token(user)
  ...
  defp maybe_set_token_authenticated_at(token, authenticated_at) do
    HeadsUp.AccountsFixtures.override_token_authenticated_at(token, authenticated_at)
  end
end
